...
@export_flags_3d_physics var collision_targets: int = 1
...
func try_attack(target: Node3D, source: Node) -> bool:
	...
	projectile.launch(global_position, target.global_position + Vector3.UP, source, collision_targets)
